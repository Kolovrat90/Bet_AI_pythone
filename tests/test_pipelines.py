...
def allocate_bank(outs: List[Outcome], bank: float) -> None:
    ...
    # итоговые ставки в € (целые), с компенсацией погрешности округления
    # рассчитываем первоначальные ставки
    stakes = [round(bank * o.f_final, 2) for o in value_outs]
    int_stakes = [int(round(s)) for s in stakes]
    total_int = sum(int_stakes)
    # желаемая сумма = округление целого bank * F_target
    desired_total = int(round(bank * F_target))
    diff = desired_total - total_int
    # компенсируем погрешность на крупнейшей ставке
    if diff != 0:
        # индекс самой большой доли f_final
...
# -----------------------------------------------------------------------------
# Fast-Screen: быстрые оценки p_est и грубый value быстрые оценки p_est и грубый value
